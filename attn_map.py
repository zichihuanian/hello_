...
def sampling_attn(text_encoder, netG, netD, netD_word, dataloader, device):
    model_dir = cfg.TRAIN.NET_G
    split_dir = 'valid'
    # Build and load the generator
    netG.load_state_dict(torch.load('../models/%s/netG_460.pth' % (cfg.CONFIG_NAME)))
    netG.eval()
    netD.load_state_dict(torch.load('../models/%s/netD_460.pth' % (cfg.CONFIG_NAME)))
    netD.eval()
    # netD_word.load_state_dict(torch.load('../models/%s/netD_word_460.pth' % (cfg.CONFIG_NAME)))
    # netD_word.eval()
    ...
    for i in range(1):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
        for step, data in enumerate(dataloader, 0):
            ...
            for j in range(batch_size):
                ...
                imp = netD.module.COND_DNET.p[j].data.cpu().numpy()
                a = netD.module.COND_DNET.p[j].data.cpu().numpy()
                # [-1, 1] --> [0, 255]
                im = (im + 1.0) * 127.5
                imp_max = imp.max()
                imp_min = imp.min()
...
if __name__ == "__main__":
    ...
    if cfg.B_VALIDATION:
        count = sampling_attn(text_encoder, netG, netD, netD_word, dataloader, device)  # generate images for the whole valid dataset
